...
def test_ne300_declares_driver_owned_status_reporting() -> None:
    assert NE300.EMITS_OWN_STATUS is True


def test_ne300_lifecycle_state_exists_before_initialize(tmp_path: Path) -> None:
    """Scheduled reads must be harmless before initialize() completes."""
    ...
    driver = NE300(
        ...
        logger=logging.getLogger("test.ecotech.ne300.lifecycle"),
        headers=NE300.HEADERS,
        parameters={},
    )

    assert driver._ne300_initialized is False
    assert isinstance(driver._logged_cursor, datetime)
    assert driver._last_logged_dtm is None
    assert driver.logged_parameter_ids == []
    assert driver.logged_packet_parameter_ids == []

    # Simulate the worker having retained an earlier initialize() failure.
    # append_record() must not replace it with a secondary AttributeError.
    driver.state.last_error = "original initialization failure"
    driver.append_record()
    assert driver.state.last_error == "original initialization failure"
