...
class Orchestrator:
    ...
    def _load_initial_configuration(self) -> None:
        ...
        schedule.every(self.application_config.main.config_reload_seconds).seconds.do(
            self._check_for_config_reload
        ).tag("main:reload")

        if self._transfer_is_enabled():
            schedule.every(self.application_config.transfer.scan_every_seconds).seconds.do(
                self._transfer_scan_all
            ).tag("main:transfer_scan")
            self.logger.info(
    ...
    def _build_network_monitor_targets(self, config: ApplicationConfig) -> list[ReachabilityTarget]:
        ...
        for name, instrument_cfg in config.instruments.items():
            ...
            targets.append(
                ReachabilityTarget(name=name, host=str(host), port=use_port, method=method)
            )
        return targets
    ...
    def _create_instrument_instance(self, instrument_config: InstrumentConfig) -> Instrument:
        ...
        try:
            ...
            raise ImportError(
                f"[{instrument_config.name}] could not resolve driver "
                f"{instrument_config.driver!r}"
            ) from exc
        ...
        # Pass the full instrument config to the driver so fields like id are available.
        ...
        try:
            ...
            raise RuntimeError(
                f"[{instrument_config.name}] failed to instantiate driver "
    ...
    def _schedule_instrument_jobs(self, instrument_config: InstrumentConfig, instrument: Instrument) -> None:
            ...
            schedule.every(instrument_config.schedule.sample_every_seconds).seconds.do(
                instrument.request_reading
            ).tag(tag)

            if instrument_config.schedule.rollover == "hourly":
                schedule.every().hour.at(instrument_config.schedule.rollover_at).do(
                    instrument.request_rollover
                ).tag(tag)
            elif instrument_config.schedule.rollover == "daily":
                schedule.every().day.at(instrument_config.schedule.rollover_at).do(
                    instrument.request_rollover
                ).tag(tag)
            ...
            if self._transfer_is_enabled() and self.application_config:
                ...
                schedule.every(instrument_config.schedule.transmit_every_seconds).seconds.do(
                    transmit_job
                ).tag(tag)
