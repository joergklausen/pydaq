...
class HMPASCII(Instrument):
    ...
    # One physical serial port is shared by all addressed sensors on that bus.
    _serial_by_port: dict[str, Any] = {}
    _serial_lock_by_port: dict[str, threading.Lock] = {}

    # Protects creation of the shared serial object and its request lock.
    # This must be distinct from the per-port request locks below: instrument
    # instances initialize concurrently, and Windows opens COM ports exclusively.
    _serial_registry_lock = threading.Lock()
    ...
    def _params(self) -> dict[str, Any]:
        ...
        if not isinstance(params, dict):
            raise ValueError(
                f"[{self.name}] missing driver parameters from orchestrator; "
                "expected a dict with 'io'."
            )
    ...
    def _optional_dict(self, payload: dict[str, Any], key: str) -> dict[str, Any]:
        ...
        if not isinstance(value, dict):
            raise ValueError(
                f"[{self.name}] invalid '{key}' configuration block; expected a mapping."
            )
        return cast(dict[str, Any], value)
    ...
    def _require_str(self, payload: dict[str, Any], key: str) -> str:
        ...
        if value is None:
            raise ValueError(
                f"[{self.name}] missing required HMPASCII configuration key '{key}'."
            )
    ...
    def _require_int(self, payload: dict[str, Any], key: str) -> int:
        ...
        try:
            ...
            raise ValueError(
                f"[{self.name}] invalid integer value for HMPASCII configuration key "
                f"'{key}': {value!r}"
            ) from exc
    ...
    def _query_measurement_socket(self) -> str:
        lock = self._socket_lock_by_endpoint[
            self._socket_endpoint_key(self.host, self.socket_port)
        ]

        with lock:
            with socket.create_connection(
                (self.host, self.socket_port), timeout=self.timeout
            ) as sock:
    ...
    def _socket_send_and_read(self, sock: socket.socket, command: str) -> str:
        ...
        data = self._socket_read_until_idle(
            sock,
    ...
    def _note_failure(self, reason: str) -> None:
        ...
        if self.fail_count >= self.max_fail_before_cooldown:
            ...
            self.logger.error(
                "[%s] repeated HMPASCII communication failures; backing off for %.0fs. "
                "Last reason: %s",
                self.name,
    ...
    def _ensure_shared_serial(self, port: str, io_cfg: dict[str, Any]) -> None:
        """Create one shared serial connection per physical port.

        Instrument instances initialize concurrently. Without the registry lock,
        two instances can both observe that a port is absent and both try to open
        it. Windows rejects the second open with ``PermissionError(13)``.
        """
        if serial is None:
            raise RuntimeError(
                f"[{self.name}] pyserial is not installed, but io.kind='serial' was "
                "configured. Install 'pyserial' or switch this instrument to "
                "io.kind='socket'."
            )

        cls = type(self)
        with cls._serial_registry_lock:
            if port in cls._serial_by_port:
                return

            # Open the port before publishing it in the shared registries. If
            # serial.Serial raises, neither dictionary contains a partial entry.
            shared_serial = serial.Serial(
                port=port,
                baudrate=int(io_cfg.get("baudrate", 19200)),
                bytesize=int(io_cfg.get("bytesize", 8)),
                parity=str(io_cfg.get("parity", "N")).upper(),
            ...
            cls._serial_by_port[port] = shared_serial
            cls._serial_lock_by_port[port] = request_lock
